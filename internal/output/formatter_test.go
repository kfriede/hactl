...
func TestPrinterFieldMask(t *testing.T) {
	...
	if _, ok := result["extra"]; ok {
		t.Error("expected field 'extra' to be filtered out")
	}
	if _, ok := result["other"]; ok {
		t.Error("expected field 'other' to be filtered out")
	}
}
...
func TestPrinterTable(t *testing.T) {
	...
	if !strings.Contains(output, "Item 1") {
		t.Error("expected table to contain 'Item 1'")
	}
}
...
func TestPrinterErrorStructured(t *testing.T) {
	...
	p.PrintError(AppError{
		...
		Guidance: "Run `hactl login` to re-authenticate.",
	})
