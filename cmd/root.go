...
var rootCmd = &cobra.Command{
	...
	Long: `hactl is a command-line interface for Home Assistant.
Built for both humans and LLM agents.

Get started:
  hactl login                        Authenticate with Home Assistant
  hactl status                       Check API connectivity
  hactl entity list                  List all entities
  hactl service call light turn_on   Call a service

Use --json for machine-readable output, --fields to select specific fields.
Run 'hactl schema <resource>.<action>' for command introspection.`,
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&flagHost, "host", "H", "", "Home Assistant URL (e.g., http://homeassistant.local:8123)")
...
func initConfig() {
	...
	if flagHost != "" {
		cfg.Host = flagHost
	} else if h := viper.GetString("host"); h != "" {
		cfg.Host = h
	}
